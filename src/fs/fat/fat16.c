...
// fat direcotry entry attributes bitmask
#define FAT_FILE_READ_ONLY 0x01
#define FAT_FILE_HIDDEN 0x02
#define FAT_FILE_SYSTEM 0x04
#define FAT_FILE_VOLUME_LABEL 0x08
#define FAT_FILE_SUBDIRECOTRY 0x10
...
struct fat_header
{
    ...
    uint8_t media_type;
    uint16_t serctor_per_fat;
    uint16_t serctor_per_track;
...
struct fat_private
{
    ...
    //Used in situation where we stream directory
...
int fat16_get_root_directory(struct disk* disk, struct fat_private* fat_private, struct fat_directory* fat_directory)
{
    int res = 0;
    struct fat_header* primary_header = &fat_private->header.primary_header;
    int root_dir_sector_pos = (primary_header->fat_copies * primary_header->serctor_per_fat) + primary_header->reserver_sectors;
...
{
    return private->header.primary_header.reserver_sectors;
}
...
static int fat16_get_fat_entry(struct disk* disk, int cluster)
{
    ...
    uint16_t results = 0;
    res = diskstreamer_read(stream, &results, sizeof(results));
    if(res < 0)
    {
        goto out;
    }
...
static int fat16_get_cluster_for_offset(struct disk* disk, int starting_cluster, int offset)
{
    int res = 0;
    struct fat_private* private = disk->fs_private;
    int size_of_cluster_bytes = private->header.primary_header.sector_per_cluster * disk->sector_size;
...
{
    println("Opening read internal function");
    int res = 0;
    struct fat_private* private = disk->fs_private;
    int size_of_cluster_bytes = private->header.primary_header.sector_per_cluster * disk->sector_size;
...
struct fat_directory* fat16_load_fat_directory(struct disk* disk, struct fat_directory_item* item)
{
    ...
    struct fat_private* fat_private = disk->fs_private;
    if(!(item->attribute & FAT_FILE_SUBDIRECOTRY))
    {
        res = -EINVARG;
        goto out;
    }
...
int fat16_seek(void* private, uint32_t offset, FILE_SEEK_MODE seek_mode)
{
    ...
    struct fat_directory_item* r_item = desc_item->item;
    if(offset >= r_item->filesize)
    {
        res = -EIO;
        goto out;
    }
