...
{
    struct process_allocation* allocation = process_get_allocation_by_addr(process, ptr_to_free);
    if(!allocation)
    {
        // Ops it not our pointer;
        return;
    } 
...
int process_map_elf(struct process* process)
{
    ...
    struct elf_header* header = elf_header(process->elf_file);
